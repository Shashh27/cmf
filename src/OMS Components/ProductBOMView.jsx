...
const ProductBOMView = ({ onBackToOrders }) => {
  ...
  const renderBOMItem = (item, level = 0) => {
    ...
    return (
      <div key={item.id}>
        <div 
          className={cn(
            "flex items-center gap-2 py-2.5 px-3 rounded-md transition-all cursor-pointer group hover:bg-blue-50 border-l-2",
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <span className={cn("text-sm font-medium truncate block", isSelected ? "text-blue-900" : "text-gray-900")}>
              {item.name}
            </span>
          </div>
        </div>
        {hasChildren && isExpanded && <div className="mt-1">{item.components.map(child => renderBOMItem(child, level + 1))}</div>}
      </div>
    );
  };
    ...
    <div className="border rounded-lg overflow-hidden">
      <table className="w-full">
        <thead className="bg-gray-100 border-b">
          <tr>{['Document Type', 'Document Name', 'Actions', 'Version'].map(h => <th key={h} className="px-4 py-3 text-left text-sm font-medium text-gray-700">{h}</th>)}</tr>
        </thead>
        <tbody>
          {documents.map((doc, idx) => (
            <tr key={doc.id || idx} className="border-b hover:bg-gray-50">
              ...
              <td className="px-4 py-3">
                <div className="flex space-x-2">
                  {[
                    ...
                    <button key={i} onClick={() => handleDocumentAction(doc.document_url, doc.document_name, btn.action)} className="p-1.5 rounded-full hover:bg-gray-200 text-gray-600 hover:text-gray-800" title={btn.title}>
              ...
              <td className="px-4 py-3">
                <select className="w-full bg-white border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" value={doc.version || '1.0'}>
  ...
  const OperationsTable = ({ operations, processPlans }) => (
    <div className="border rounded-lg overflow-hidden">
      <table className="w-full">
        <thead className="bg-gray-100 border-b">
          <tr>{['Operation #', 'Operation Name', 'Setup Time', 'Cycle Time', 'Workcenter'].map(h => <th key={h} className="px-4 py-3 text-left text-sm font-medium text-gray-700">{h}</th>)}</tr>
        </thead>
        <tbody>
                ...
                <tr className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => toggleOperationExpand(op.id)}>
                  <td className="px-4 py-3">
                    <div className="flex items-center">
                      {plan ? (isExpanded ? <ChevronDown size={16} className="mr-2" /> : <ChevronRight size={16} className="mr-2" />) : <span className="w-4 mr-2" />}
                    ...
                    <td colSpan="5" className="px-4 py-4">
                      <div className="pl-8 space-y-3">
                        <div>
                          ...
                          <p className="text-sm text-gray-700 whitespace-pre-line">{plan.work_instructions || 'No work instructions provided.'}</p>
                        </div>
                        {plan.notes && (
                          <div>
                            <h4 className="font-semibold text-sm mb-2">Notes</h4>
                            <p className="text-sm text-gray-700 whitespace-pre-line">{plan.notes}</p>
                          </div>
                        )}
                      </div>
                    </td>
  ...
  const renderDetailsPanel = () => {
    ...
    if (!isPart) return <div className="text-center py-12"><p className="text-gray-400">Select a part to view {bomView === 'ebom' ? 'documents' : 'operations'}</p></div>;

    if (bomView === 'ebom') {
      return (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold mb-3">Documents ({selectedItem.documents?.length || 0})</h3>
    ...
    return (
      <div className="space-y-4">
        <h3 className="text-lg font-semibold mb-2">Operations ({selectedItem.operations?.length || 0})</h3>
    ...
    return (
      <div className="container mx-auto p-4">
        <div className="flex items-center mb-4">
          <Button variant="outline" size="sm" disabled><ArrowLeft className="h-4 w-4 mr-2" />Back to Orders</Button>
  ...
  if (!bomData) {
    return (
      <div className="container mx-auto p-4">
        <div className="flex items-center mb-4">
          <Button variant="outline" size="sm" onClick={onBackToOrders}><ArrowLeft className="h-4 w-4 mr-2" />Back to Orders</Button>
    ...
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center space-x-2">
          {['mbom', 'ebom'].map(view => (
            <Button key={view} variant={bomView === view ? 'default' : 'outline'} size="sm" onClick={() => setBomView(view)}>{view.toUpperCase()}</Button>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="w-full lg:w-1/3">
          <Card>
            <CardHeader><CardTitle>BOM Structure</CardTitle></CardHeader>
            <CardContent>
              <ScrollArea className="h-[calc(100vh-280px)]">{bomData && renderBOMItem(bomData)}</ScrollArea>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex-1">
          <Card>
            <CardHeader className="bg-gray-50">
              ...
              {selectedItem && <p className="text-sm text-gray-600 uppercase">{selectedItem.type}</p>}
            </CardHeader>
            <CardContent className="pt-4">
              <ScrollArea className="h-[calc(100vh-280px)]">{renderDetailsPanel()}</ScrollArea>
            </CardContent>
          </Card>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl max-h-[90vh] w-full mx-4">
            <div className="flex items-center justify-between p-4 border-b">
              <h3 className="text-lg font-semibold">{documentModal.name || 'Document'}</h3>
