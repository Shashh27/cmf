                ...
                {versions.map(ver => (
                  <div key={ver.id} className="flex justify-between items-center bg-white px-3 py-2 border border-gray-200 shadow-sm">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Tooltip title="Download"><Button size="small" type="text" icon={<DownloadOutlined />} onClick={async () => {
                        try {
                          await downloadBlobWithAuth(`/operation-documents/${ver.id}/download`, ver.document_name);
                        } catch (e) {
                          console.error(e);
                        }
                      }} className="text-green-500 hover:bg-green-50" /></Tooltip>
                    </div>
                  </div>
                ))}
              </div>
...
// ── DocumentsPanel ──────────────────────────────────────────────────────────
const DocumentsPanel = ({ selectedItem, onDocumentsLoaded, compactMode = false, onTabChange, externalActiveTab, denseMode = false, readOnly = true }) => {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      // Filter documents: PC uploads require acknowledgment; admin/MC/QA uploads are visible without a release step
      const filteredDocs = docs.filter(doc => {
        ...
        // Admin / MC / QA uploads are visible to everyone (QA is auto-acknowledged on upload)
        return true;
      });
  ...
  const handleDeleteDocument = async (id) => {
    try {
      ...
      if (readOnly && currentUserId != null && doc && Number(doc.user_id) !== Number(currentUserId)) {
        message.error('You can only delete documents you uploaded.');
        return;
      }
      ...
      Modal.confirm({
        ...
        onOk: async () => {
          try {
            ...
            await api.post(`/operations/swap`, null, {
              params: {
                op1_id: sourceOp.id,
                op2_id: targetOp.id
              }
  ...
  const operationsColumns = [
    ...
    { title: <span className="font-semibold text-slate-700">To Date</span>, dataIndex: 'to_date', key: 'to', width: 120,
      render: v => v ? <span className="text-sm text-slate-700 font-medium">{new Date(v).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span> : <span className="text-slate-500">—</span> },
    { title: <span className="font-semibold text-slate-700 text-center block">Actions</span>, key: 'actions', align: 'center', width: readOnly ? 60 : 150, fixed: 'right',
      render: (_, r) => {
  ...
  const eBomColumns = [
    ...
    { title: <span className="text-xs font-semibold">ACKNOWLEDGED</span>, key: 'acknowledged', width: 150, align: 'center',
      render: (_, r) => {
        ...
        if (cur.mc_is_rejected) {
          ...
          return (
            <Tooltip title={cur.mc_ack_remarks || 'Acknowledged'}>
    ...
    { title: <span className="text-xs font-semibold text-center block">ACTIONS</span>, key: 'actions', width: 160, align: 'center', fixed: 'right',
      render: (_, r) => {
        const cur = selectedVersions[String(r.parent_id ?? r.id)] || r;
        const currentUserId = getCurrentUserId();
        const isOwnDoc = currentUserId != null && Number(cur.user_id) === Number(currentUserId);
        const qualityDoc = isQualityDocument(cur);
        const canMutateDoc = !qualityDoc && (!readOnly || isOwnDoc);
        return (
          <div className="flex gap-1 justify-center">
      ...
      children: (
        <div className="h-full flex flex-col min-h-0">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-1.5 shrink-0 gap-2">
            <span className="text-xs text-slate-500">{readOnly ? 'View operations' : 'Drag rows to reorder operations'}</span>
        ...
        <div className="h-full flex flex-col min-h-0 overflow-hidden">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 shrink-0 gap-2">
            ...
            <Button type="primary" size="small" icon={<PlusOutlined />} className="primary-btn-sm w-full sm:w-auto"
              onClick={() => { resetUploadState(); setIsUploadModalOpen(true); }}>
              Add Document
            </Button>
          </div>
