...
const BillOfMaterials = ({ onItemSelected, onHierarchyLoaded, disableProductCreate = false, readOnly = true, initialProductId = null, bomRefreshTrigger = 0 }) => {
  ...
  const ActionButtons = ({ item, type, tagName, tagColor }) => {
    ...
    // QA is view-only: no add / edit / delete / upload part actions
    const getMenuItems = () => {
      if (readOnly) return [];
  ...
  const renderPartInTree = (part, level = 0, productId = null) => {
    ...
    return (
      <DraggablePartRow
        key={`part-${part.id}`}
        partId={part.id}
        disabled={readOnly || isInRecycleBin}
