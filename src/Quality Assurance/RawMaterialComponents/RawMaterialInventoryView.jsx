...
import { Spin, Empty, App, Badge } from "antd";
...
const RawMaterialInventoryView = ({
  ...
  const rows = useMemo(() => {
    ...
    const highlightId = highlightStockId != null && highlightStockId !== ""
      ? Number(highlightStockId)
      : null;
    ...
    inventoryData.forEach((material) => {
      if (matArr.length > 0 && !matArr.includes(material.id)) {
        // Still show material if it owns the highlighted stock
        const ownsHighlight = highlightId != null && (allStock[material.id] || []).some((s) => Number(s.id) === highlightId);
      ...
      const matchingStocks = stocks.filter((s) => {
        if (highlightId != null && Number(s.id) === highlightId) return true;
    ...
    return result;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [inventoryData, allStock, allUnits, searchText, fMaterialKey, fSourceKey, fOrderKey, fPartKey, fStockStatusKey, fUnitStatusKey, colProcess, colForm, colSource, colStockStatus, colUnitStatus, highlightStockId]);

  useEffect(() => { if (onRowsReady) onRowsReady(rows); }, [rows, onRowsReady]);

  useEffect(() => {
    if (!highlightStockId || !rows.length) return;
    const timer = setTimeout(() => {
      const el = document.querySelector(`[data-highlight-stock-id="${highlightStockId}"]`);
      if (el) {
        el.scrollIntoView({ behavior: "smooth", block: "center" });
      }
    }, 250);
    return () => clearTimeout(timer);
  }, [highlightStockId, rows]);
            ...
            <tbody>
              {rows.map((row, idx) => {
                const isHighlighted =
                  highlightStockId != null &&
                  row.stock &&
                  Number(row.stock.id) === Number(highlightStockId);
                const stockHl = isHighlighted
                  ? { background: "#fff7e6", boxShadow: "inset 0 0 0 1px #ffd591" }
                  : {};
                const rowBg = idx % 2 === 0 ? "#fff" : "#fafafa";
                return (
                <tr
                  key={idx}
                  data-highlight-stock-id={isHighlighted ? String(highlightStockId) : undefined}
                ...
                >
                  ...
                  {row.type === "no-stock" ? (
                    <td colSpan={15} style={{ ...tdStyle, color: "#aaa", fontStyle: "italic" }}>No stock available</td>
                  ) : (
                    <>
                      {row.stockRowSpan > 0 && (
                        <>
                          <td rowSpan={row.stockRowSpan} style={{ ...tdStyle, ...stockHl }}>{row.stock.process_type || "-"}</td>
                          <td rowSpan={row.stockRowSpan} style={{ ...tdStyle, ...stockHl }}>{row.stock.form_type || "-"}</td>
                          <td rowSpan={row.stockRowSpan} style={{ ...tdStyle, fontFamily: "monospace", fontSize: Math.max(7, density.fontSize - 1), ...stockHl }}>{fmtDim(row.stock)}</td>
                          <td rowSpan={row.stockRowSpan} style={{ ...tdStyle, ...stockHl }}>{row.stock.quantity ?? "-"}</td>
                          <td rowSpan={row.stockRowSpan} style={{ ...tdStyle, ...stockHl }}>{row.stock.mass != null ? row.stock.mass.toFixed(2) : "-"}</td>
                          <td rowSpan={row.stockRowSpan} style={{ ...tdStyle, ...stockHl }}>{row.stock.source_type === "order" ? "Order" : "Gen"}</td>
                          <td rowSpan={row.stockRowSpan} style={{ ...tdStyle, ...stockHl }}>
                      ...
                      {row.type === "no-unit" ? (
                        <td colSpan={7} style={{ ...tdStyle, color: "#aaa", fontStyle: "italic", ...stockHl }}>No units</td>
                      ) : (
                        <>
                          <td style={{ ...tdStyle, ...stockHl }}>
                            {row.stock.source_type === "order"
                              ? (row.stock.source_order_number || "-")
                              : (getUnitOrders(row.unit, row.stock)[0] || "-")}
                          </td>
                          <td style={{ ...tdStyle, ...stockHl }}>{density.compact ? row.unitSeq : `U${row.unitSeq}`}</td>
                          <td style={{ ...tdStyle, ...stockHl }}>{row.unit.total_length?.toFixed(1) ?? "-"}</td>
                          <td style={{ ...tdStyle, ...stockHl }}>{row.unit.remaining_length?.toFixed(1) ?? "-"}</td>
                          <td style={{ ...tdStyle, textAlign: "left", ...stockHl }}>
