...
const EditOperationModal = ({
  ...
  const [preview, setPreview]                   = useState(null); // { url, title, type, id, name }
  ...
  // Fetch preview content with JWT (img/iframe/video tags cannot send the Bearer header).
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await api.get(`/operation-documents/${preview.id}/preview`, { responseType: 'blob' });
        if (cancelled) return;
