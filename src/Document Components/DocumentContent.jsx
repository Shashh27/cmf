...
const DocumentContent = ({ selectedNode }) => {
  ...
  const fetchDocuments = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://172.18.7.89:8000/general-documents/folders/${selectedNode.folderId}/documents`);
      if (!response.ok) {
  ...
  const handleUpdateDocumentName = async () => {
    ...
    try {
      const response = await fetch(`http://172.18.7.89:8000/general-documents/documents/${editingDocument.id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleDownloadDocument = async (document) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.89:8000/general-documents/documents/${document.id}/download`, {
        method: 'GET',
  ...
  const handleDeleteDocument = (document) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await fetch(`http://172.18.7.89:8000/general-documents/documents/${document.id}`, {
            method: 'DELETE'
          });
