...
const OMS = () => {
  ...
  const tableColumnFilters = useMemo(() => {
    const projectCoordinatorFilters = Array.from(
      new Set(
        orders
    ...
    return { projectCoordinatorFilters, approvalStatusFilters, statusFilters, adminFilters, customerFilters };
  }, [orders]);
  ...
  const columns = [
        ...
        record.approval_status === "Rejected" ? (
          <Tooltip title="Order rejected - cannot access project">
            <Space className="text-gray-400" size={2}>
              ...
              <span className="font-medium text-xs truncate">{record.product_name || `Project ${productId}`}</span>
            </Space>
          </Tooltip>
        ) : (
          <Button
          ...
          >
            <Space className="text-gray-700" size={2}>
              ...
              <span className="underline text-xs truncate">{record.product_name || `Project ${productId}`}</span>
            </Space>
          </Button>
        )
      ),
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space size={2}>
          <UserOutlined className="text-gray-400 text-xs" />
          <span className="text-gray-700 text-xs truncate">{text || record.admin_id || "-"}</span>
        </Space>
      ),
    },
    {
      ...
      width: 140,
      ellipsis: true,
      filters: tableColumnFilters.projectCoordinatorFilters,
