...
import RawMaterialInventoryAnalytics from "../../RawMaterialComponents/RawMaterialInventoryAnalytics";
...
const RawMaterialInventoryTab = ({ rawMaterials = [] }) => {
  ...
  return (
    <div className="mt-4 w-full overflow-hidden">
      <div className="bg-white rounded-lg lg:rounded-xl shadow-sm border border-gray-100 p-2 sm:p-3 mb-4">
            ...
            {viewMode === "table" && (
              <>
                <Input.Search
                  ...
                  size="middle"
                />
                <Select mode="multiple" placeholder="Material" allowClear showSearch optionFilterProp="children" value={invFilters.fMaterial} onChange={v => setF('fMaterial', v)} className="min-w-[120px] max-w-full sm:max-w-[260px] flex-1 sm:flex-none" size="middle" maxTagCount={1} maxTagPlaceholder={(omitted) => `+${omitted.length} more`}>
                  {invFilterOptions.materials.map(m => <Option key={m.id} value={m.id}>{m.name}</Option>)}
                </Select>
                <Select mode="multiple" placeholder="Source" allowClear value={invFilters.fSource} onChange={v => { setInvFilters(p => ({ ...p, fSource: v || [], fOrder: [], fPart: [] })); }} className="min-w-[100px] max-w-full sm:max-w-[200px] flex-1 sm:flex-none" size="middle" maxTagCount={1} maxTagPlaceholder={(omitted) => `+${omitted.length} more`}>
                  <Option value="general">General</Option>
                  <Option value="order">Order</Option>
                </Select>
                <Select mode="multiple" placeholder="Order No" allowClear showSearch optionFilterProp="children" value={invFilters.fOrder} onChange={v => { setInvFilters(p => ({ ...p, fOrder: v || [], fPart: [] })); }} className="min-w-[120px] max-w-full sm:max-w-[260px] flex-1 sm:flex-none" size="middle" maxTagCount={1} maxTagPlaceholder={(omitted) => `+${omitted.length} more`} disabled={invFilters.fSource.length > 0 && !invFilters.fSource.includes('order')}>
                  {invFilterOptions.orders.map(o => <Option key={o} value={o}>{o}</Option>)}
                </Select>
                <Select mode="multiple" placeholder="Part No" allowClear showSearch optionFilterProp="children" value={invFilters.fPart} onChange={v => setF('fPart', v)} className="min-w-[110px] max-w-full sm:max-w-[260px] flex-1 sm:flex-none" size="middle" maxTagCount={1} maxTagPlaceholder={(omitted) => `+${omitted.length} more`} disabled={invFilters.fOrder.length === 0}>
                  {Array.from(new Set(invFilters.fOrder.flatMap(o => invFilterOptions.partsByOrder[o] || []))).sort().map(p => <Option key={p} value={p}>{p}</Option>)}
                </Select>
                <Select mode="multiple" placeholder="Stock Status" allowClear value={invFilters.fStockStatus} onChange={v => setF('fStockStatus', v)} className="min-w-[120px] max-w-full sm:max-w-[240px] flex-1 sm:flex-none" size="middle" maxTagCount={1} maxTagPlaceholder={(omitted) => `+${omitted.length} more`}>
                  <Option value="available">Available</Option>
                  <Option value="not_available">Not Available</Option>
                  <Option value="exhausted">Exhausted</Option>
                </Select>
                <Select mode="multiple" placeholder="Unit Status" allowClear value={invFilters.fUnitStatus} onChange={v => setF('fUnitStatus', v)} className="min-w-[120px] max-w-full sm:max-w-[240px] flex-1 sm:flex-none" size="middle" maxTagCount={1} maxTagPlaceholder={(omitted) => `+${omitted.length} more`}>
                  <Option value="available">Available</Option>
