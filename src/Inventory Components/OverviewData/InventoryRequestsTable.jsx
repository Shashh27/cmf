...
const InventoryRequestsTable = () => {
  ...
  const fetchInventoryRequests = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://172.18.7.89:8000/api/v1/inventory-requests/');
      if (!response.ok) {
  ...
  const handleApprove = async (record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await fetch(`http://172.18.7.89:8000/api/v1/inventory-requests/${record.id}/status?admin_id=6&status=approved`, {
            method: 'PUT'
          });
  ...
  const handleReject = async (record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await fetch(`http://172.18.7.89:8000/api/v1/inventory-requests/${record.id}/status?admin_id=6&status=rejected`, {
            method: 'PUT'
          });
