...
const ReturnRequestsTable = () => {
  ...
  const fetchReturnRequests = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://172.18.7.89:8000/api/v1/inventory-return-requests/');
      if (!response.ok) {
  ...
  const handlePending = async (record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        console.log('API URL:', `http://172.18.7.89:8000/api/v1/inventory-return-requests/${record.id}/status?admin_id=6&status=pending&table_id=${record.id}`);
        
        try {
          const response = await fetch(`http://172.18.7.89:8000/api/v1/inventory-return-requests/${record.id}/status?admin_id=6&status=pending&table_id=${record.id}`, {
            method: 'PUT'
          });
  ...
  const handleCollected = async (record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        console.log('Record Tool Name:', record.inventory_request_details?.tool_name);
        console.log('Current Status:', record.status);
        console.log('API URL:', `http://172.18.7.89:8000/api/v1/inventory-return-requests/${record.id}/status?admin_id=6&status=collected&table_id=${record.id}`);
        
        try {
          const response = await fetch(`http://172.18.7.89:8000/api/v1/inventory-return-requests/${record.id}/status?admin_id=6&status=collected&table_id=${record.id}`, {
            method: 'PUT'
          });
  ...
  const handleDelete = async (record) => {
    try {
      const response = await fetch(`http://172.18.7.89:8000/api/v1/inventory-return-requests/${record.id}`, {
        method: 'DELETE'
      });
